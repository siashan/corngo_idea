 ...
 */
function addGoods(){
    var url = inc.ctx + "/taobao/goods/toAdd";
...
function editGoods() {
    if (inc.checkSelected(gridObj)) {
        var url = inc.ctx + '/taobao/goods/toEdit/' + inc.getSelectedRowDate(gridObj, "id");
        window.location.href = url;
    }else{
...
function delGoods() {
    if (inc.checkSelected(gridObj)) {
        window.parent.layer.confirm('您确定要删除该记录吗？', {
            ...
            var url = inc.ctx + "/taobao/goods/del/"+ inc.getSelectedRowDate(gridObj,"id");
...
function getType(record, rowIndex, colIndex, options){
    var my = gridObj.getRecordIndexValue(record, 'type');
    return eval('goodsType.d_' + my);
}
