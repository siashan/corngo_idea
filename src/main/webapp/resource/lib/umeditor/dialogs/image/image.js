(function () {
    ...
    var utils = UM.utils,
        ...
        getAllPic: function (sel, $w, editor) {
            ...
            $.each($imgs, function (index, node) {
                ...
                var url = $(node).data("url");  //fix by kfc@2015.4.27 取配置路径+图片相对路径
                return arr.push({
                    //_src: url,
                    src: url
                });
        ...
        callback: function (editor, $w, url, state) {
            ...
            if (state == "SUCCESS") {
                //显示图片计数+1
                Upload.showCount++;
                var $img = $("<img src='" + editor.options.imagePath + url + "' data-url='" + editor.options.imagePath + url + "' class='edui-image-pic' />"),
    ...
    var Upload = {
        ...
        config: function (sel) {
            ...
            $("form", $(sel, me.dialog)).attr("action", url+'&r='+Math.random());

            return me;
        },
        uploadComplete: function(r){
            var me = this;
            var rr = $(r).text();  // fix by kfc 删除自带的P标记 20140310
            try{
                var json = eval('('+rr+')');
                //var json = eval('('+r+')');
